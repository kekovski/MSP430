...
// The size of a TFTP block - 512 bytes (decimal format)
#define TFTP_BLOCK_SIZE_DECIMAL (512L)
...
void TFTPOpen(IP_ADDR *host)
{
    ...
    //TODO: Originally the following line was commented
...
TFTP_RESULT TFTPIsOpened(void)
{
    ...
    case SM_TFTP_WAIT:
        ...
        {
            _tftpSocket = UDPOpenEx((DWORD)(&MutExVar.group1._hostInfo),
            						UDP_OPEN_NODE_INFO,
            						TFTP_CLIENT_PORT,
                                  TFTP_SERVER_PORT);

            if(UDPSocketInfo[_tftpSocket].smState == UDP_OPENED)
 ...
 *                  timeout, function returns with TFTP_RETRY
 ...
 *                  TFTP_TIMEOUT.
 *
 * Note:            By default, this function uses "octet" or binary
 *                  mode of file transfer.
 ********************************************************************/
...
BYTE TFTPGet(void)
{
    ...
    if (MutExVar.group2._tftpBlockLength.Val == TFTP_BLOCK_SIZE_DECIMAL) {
