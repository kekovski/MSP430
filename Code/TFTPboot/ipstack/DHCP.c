...
typedef struct {
	...
	//TODO: Modified for option 150
	IP_ADDR tempTFTP; //Temporary TFTP server to use when no DHCP lease
	WORD tempBootFileSize; //Temporary boot file size(in units of 512B) to use when no DHCP lease
	BYTE tempBootFileName[255]; //Temporary boot file name to use when no DHCP lease
	...
	union {
		struct {
			...
			//TODO: Modified for option 150
			char TFTP :1; // Primary TFTP server address is valid
			char BootFileName :1; //The name of the boot file is valid
			char BootFileSize :1; //The size of the boot file is valid
...
/*****************************************************************************
 ...
 Summary:
 Determines if the DHCP client on the specified interface has seen a DHCP
 server.
...
void DHCPTask(void) {
	...
	for (i = 0; i < NETWORK_INTERFACES; i++) {
		...
		case SM_DHCP_GET_SOCKET:
			...
			DHCPClient.hDHCPSocket = UDPOpenEx(0, UDP_OPEN_SERVER,
		...
		case SM_DHCP_SEND_DISCOVERY:
			...
			memset(
					(void*) &UDPSocketInfo[DHCPClient.hDHCPSocket].remote.remoteNode,
					0xFF, sizeof(UDPSocketInfo[0].remote.remoteNode));
		...
		case SM_DHCP_GET_REQUEST_ACK:
			...
			case DHCP_ACK_MESSAGE:
				...
				if (DHCPClient.validValues.bits.TFTP && DHCPClient.validValues.bits.BootFileName && DHCPClient.validValues.bits.BootFileSize){
					AppConfig.PrimaryTFTPServer = DHCPClient.tempTFTP;
					AppConfig.BootFileName = DHCPClient.tempBootFileName;
					AppConfig.BootFileSize = DHCPClient.tempBootFileSize;

					//We already have all the TFTP parameters, so we consider TFTP configured already
					AppConfig.Flags.bTFTPInConfigMode = FALSE;
				}
...
static BYTE _DHCPReceive(void) {
	...
	if (v == BOOT_REPLY) {
		...
		if (!DHCPClient.flags.bits.bOfferReceived) {
			UDPSetRxBuffer(16);
			UDPGetArray((BYTE*) &DHCPClient.tempIPAddress,
					sizeof(DHCPClient.tempIPAddress));
		...
		do {
			...
			case DHCP_IP_LEASE_TIME:
				...
				//TODO: Changed code to allow option 150(TFTP server), as specified in RFC5859
			case DHCP_TFTP_SERVER_ADDRESS:
				UDPGet(&v); //Get len
				//Check if len is a multiple of 4B(size of IP address)
				if (v >= 4 && (v%4) == 0) {

					//Check if we already have an offer with a valid TFTP server
					//and in that case dismiss this option
					if (DHCPClient.flags.bits.bOfferReceived) {
						for (i = v; i > 0; i--)
							UDPGet(&v);
					}
						...
						//The RFC allows to include any number of servers in decreasing priority
						//but we'll just take the first one(highest priority) and ignore the rest, if any
						UDPGetArray((BYTE*) &DHCPClient.tempTFTP,
								sizeof(DHCPClient.tempTFTP));
						DHCPClient.validValues.bits.TFTP = 1;

						//Discard the other addresses, if any
						for (i = v - 4; i > 0; i--)
							UDPGet(&v);
		    ...
		    //TODO: Modified for option 13
			case DHCP_BOOT_FILE_SIZE:
				UDPGet(&v); //Get length in bytes

				//The only allowed length of this field is 2 bytes
				if (v == 2) {
					//Check if we already received an offer, and in that case dismiss this option
					if (DHCPClient.flags.bits.bOfferReceived) {
						for (i = v; i > 0; i--)
							UDPGet(&v);
					} else {
						//Get the string of bytes
						UDPGetArray((BYTE*)&DHCPClient.tempBootFileSize, sizeof(DHCPClient.tempBootFileSize));
						DHCPClient.validValues.bits.BootFileSize = 1;
					}
				} else
					goto UDPInvalid;
				break;

				//TODO: Modified for option 67
			case DHCP_BOOT_FILE_NAME:
				UDPGet(&v); //Get length in bytes

				//Minimum length for this option is 1
				if (v > 1) {
					//Check if we already received an offer, and in that case dismiss this option
					if (DHCPClient.flags.bits.bOfferReceived) {
						for (i = v; i > 0; i--)
						...
						//Get the string of bytes
						UDPGetArray(DHCPClient.tempBootFileName, v);
						//Insert a string terminator just in case
						DHCPClient.tempBootFileName[v]='\0';
						DHCPClient.validValues.bits.BootFileName = 1;
					}
				} else
					goto UDPInvalid;
				break;
...
static void _DHCPSend(BYTE messageType, BOOL bRenewing) {
	...
	if ((messageType == DHCP_REQUEST_MESSAGE) && bRenewing) {
		UDPPutArray((BYTE*) &DHCPClient.tempIPAddress,
				sizeof(DHCPClient.tempIPAddress));
	} else {
		UDPPut(0x00);
		UDPPut(0x00);
	...
	//TODO: Modified for option 150
	UDPPut(DHCP_TFTP_SERVER_ADDRESS);
	UDPPut(DHCP_BOOT_FILE_SIZE);
	UDPPut(DHCP_BOOT_FILE_NAME);

	// Add requested IP address to DHCP Request Message
	if (((messageType == DHCP_REQUEST_MESSAGE) && !bRenewing)
			|| ((messageType == DHCP_DISCOVER_MESSAGE)
					&& DHCPClient.tempIPAddress.Val)) {
		UDPPut(DHCP_PARAM_REQUEST_IP_ADDRESS);
		UDPPut(DHCP_PARAM_REQUEST_IP_ADDRESS_LEN);
		UDPPutArray((BYTE*) &DHCPClient.tempIPAddress,
				DHCP_PARAM_REQUEST_IP_ADDRESS_LEN);
	}
